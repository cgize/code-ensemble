...
export function isQuotaError(error: unknown): boolean {
  ...
  return /\b(quota exceeded|insufficient_quota|rate limit|too many requests|usage limit|capacity exceeded)\b/i.test(
    errorText(error),
  );
}
...
export async function delegateWithQuotaFallback(
  client: FallbackClient,
  input: {
    parentSessionID: string;
    description: string;
  ...
  try {
    ...
    if (!isQuotaError(error) || !input.fallbackModel) throw error;
    ...
    try {
      ...
      throw new Error(
        `Quota fallback failed for ${input.role}. Tried ${primary.model} then ${fallback.model}: ${errorText(fallbackError)}`,
      );
    }
  }
}
