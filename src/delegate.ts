...
function payload(input: Record<string, unknown>): string {
...
async function delay(milliseconds: number, signal?: AbortSignal): Promise<void> {
  if (signal?.aborted) throw signal.reason ?? new Error("Delegation aborted");
...
export class FallbackDelegator {
  ...
  start(input: StartInput): { taskID: string; output: string } {
    if (this.disposed) throw new Error("Delegator is disposed");
  ...
  private async run(
    ...
    const timeout = setTimeout(
      () => controller.abort(new DelegationTimeoutError(`Delegation timed out after ${DELEGATION_TIMEOUT_MS / 60_000} minutes`)),
      DELEGATION_TIMEOUT_MS,
    );
    timeout.unref?.();
    let state: "completed" | "error";
    let resultPayload: Record<string, unknown>;
    let timedOut = false;
    try {
      const result = await raceWithAbort(
        delegateWithFallback(this.client, { ...input, signal: controller.signal }),
        controller.signal,
      );
      state = "completed";
      resultPayload = {
  ...
  private async deliver(
    ...
    while (Date.now() - started < DELIVERY_RETRY_WINDOW_MS) {
      try {
        if (await this.parentIsBusy(parentSessionID, signal)) throw new Error("Parent session is busy");
  ...
  private async parentIsBusy(parentSessionID: string, signal?: AbortSignal): Promise<boolean> {
    try {
